...
def get_smsf_query_engine(fund_id: str, vector_index):
    ...
    # Configure global settings for the entire pipeline
    llm = OpenAI(model="gpt-4o-mini")
    Settings.llm = llm

    # Helper to build the underlying engine for each tool using RetrieverQueryEngine
    def create_compliant_engine(filters):
        # .as_query_engine() builds a RetrieverQueryEngine under the hood
    ...
    fallback_tool = QueryEngineTool(
        ...
        metadata=ToolMetadata(
            name="fallback",
            description="Use this ONLY for greetings (hello/hi) or general questions about superannuation."
        )
    )
